...
public class Ordenacion {
    ...
    */
    ...
    public static <T extends Comparable<T>> void mergeSort2(T[] v) {
        ...
        for(int i = 0; i < v.length; i++) v[i] = res[i];
    }
    ...
    private static <T extends Comparable<T>> T[] mergeSort2(T[] v,
                                                             int i, int f) {
        if (i < f) {
            // Divide de forma recursiva el array por la mitad
            int m = (i + f) / 2;
            T[] v1 = mergeSort2(v, i, m);
            T[] v2 = mergeSort2(v, m + 1, f);
            return merge2(v1, v2);
        } //Para volver a unirlo de forma ordenada
        T[] res = (T[]) new Comparable[1];
        res[0] = v[i]; // En caso de que solo sea un elemento, ya está ordenado
        return res;
    }        
    ...
    private static <T extends Comparable<T>> T[] merge2(T[] v1, T[] v2) {
        int m = v1.length-1; // Mitad
        int f = v2.length -1; // Final
        
        int a = 0; // Posición actual primer array
        int b = 0; // Posición actual segundo array
        int k = 0; // Posición actual del nuevo array
        T[] aux = (T[]) new Comparable[v1.length + v2.length];
        
        while (a <= m && b <= f) { // Mientras ningún array termine...
